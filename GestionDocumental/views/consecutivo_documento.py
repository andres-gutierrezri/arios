...
class ConsecutivoDocumentoCrearView(AbstractEvaLoggedView):
    def get(self, request):
        contratos = Contrato.objects.get_xa_select_activos()
        return render(request, 'GestionDocumental/ConsecutivoDocumento/crear.html', {'fecha': datetime.datetime.now(),
                                                                                     'contratos': contratos,
                                                                                     'menu_actual': 'consecutivos'})

    def post(self, request):
        consecutivo = ConsecutivoDocumento.from_dictionary(request.POST)

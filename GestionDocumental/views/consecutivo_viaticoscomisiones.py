...
class ConsecutivoViaticosComisionesView(AbstractEvaLoggedView):
    def get(self, request, id):
        if id == 0:
            ...
            consecutivos = ConsecutivoViaticosComisiones.objects.filter(usuario_crea_id=request.user.id,
...
class ConsecutivoViaticosComisioneCrearView(AbstractEvaLoggedView):
    ...
    def post(self, request):
        ...
        try:
            ...
            rollback()
            LOGGER.exception('Falló la generación del consecutivo de actas de viáticos y comisiones.')
            return RespuestaJson.error('Ha ocurrido un error al crear el consecutivo.')


class ConsecutivoViaticosComisioneEditarView(AbstractEvaLoggedView):
    def get(self, request, id):
        consecutivo = ConsecutivoViaticosComisiones.objects.get(id=id)
        return render(request, 'GestionDocumental/ConsecutivoViaticosComisiones/_modal_crear_editar_consecutivo.html',
    ...
    def post(self, request, id):
        ...
        consecutivo = ConsecutivoViaticosComisiones.from_dictionary(request.POST)
        consecutivo_db = ConsecutivoViaticosComisiones.objects.get(id=id)
        ...
        if consecutivo_db.comparar(consecutivo, excluir=['usuario_crea', 'fecha_crea', 'fecha_modificacion',
            ...
            try:
                ...
                rollback()
                LOGGER.exception('Falló la edición del consecutivo de viáticos y comisiones.')
                RespuestaJson.error('Falló la edición del consecutivo de viáticos y comisiones.')


class ConsecutivoViaticosComisioneEliminarView(AbstractEvaLoggedView):
    def post(self, request, id):
        consecutivo = ConsecutivoViaticosComisiones.objects.get(id=id)
        ...
        try:
            ...
            rollback()
            LOGGER.exception('Error anulando el consecutivo de viáticos y comisiones')
            return RespuestaJson.error('Ha ocurrido un error al realizar la acción')


def datos_xa_render(request, consecutivo: ConsecutivoViaticosComisiones = None) -> dict:

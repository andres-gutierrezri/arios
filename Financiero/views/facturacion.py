...
class FacturasView(AbstractEvaLoggedView):
    def get(self, request):
        if not tiene_permisos(request, 'Financiero', ['view_facturaencabezado'], None):
...
class FacturaCrearView(AbstractEvaLoggedView):
    def get(self, request):
        if not tiene_permisos(request, 'Financiero', ['add_facturaencabezado'], None):
            return redirect(reverse('eva-index'))
        else:
    ...
    def post(self, request):
        if not tiene_permisos(request, 'Financiero', ['add_facturaencabezado', 'change_facturaencabezado'], None):
            return redirect(reverse('eva-index'))
        else:
...
class FacturaEditarView(AbstractEvaLoggedView):
    def get(self, request, id_factura):
        if not tiene_permisos(request, 'Financiero', ['change_facturaencabezado'], None):
            return redirect(reverse('eva-index'))
        else:
            terceros = Tercero.objects.clientes_xa_select()
            impuestos = Impuesto.objects.get_xa_select_porcentaje()

...
class ColaboradoresIndexView(AbstractEvaLoggedView):
    ...
    def get(self, request, id_contrato):
        if not tiene_permisos(request, 'TalentoHumano', ['view_colaborador'], None):
...
class ColaboradoresCrearView(AbstractEvaLoggedView):
    ...
    def get(self, request):
        if not tiene_permisos(request, 'TalentoHumano', ['add_colaborador'], None):
            return redirect(reverse('eva-index'))
        else:
...
class ColaboradorEditarView(AbstractEvaLoggedView):
    ...
    def get(self, request, id):
        if not tiene_permisos(request, 'TalentoHumano', ['change_colaborador'], None):
            return redirect(reverse('eva-index'))
        else:
            colaborador = Colaborador.objects.get(id=id)
...
class ColaboradorEliminarView(AbstractEvaLoggedView):
    def post(self, request, id):
        if not tiene_permisos(request, 'TalentoHumano', ['delete_colaborador'], None):
            return redirect(reverse('eva-index'))
        else:
            try:
                colaborador = Colaborador.objects.get(id=id)
